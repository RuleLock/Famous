 ...
 * @date 2017/11/10.
 */
class DetailsAdapter(private var ctx: Context, private var list: List<Int>): BaseAdapter() {

    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        var convert = convertView
        val holder: ViewHolder
        if (convert == null) {
            convert = LayoutInflater.from(ctx).inflate(R.layout.item_details, parent, false)
